...
rss_headers = nonebot.get_driver().config.headers
rssdb = RSSDB(rss_db_path)

async def fetch_feed(url: str):
    async with aiohttp.ClientSession() as client:
        async with client.get(url, proxy=proxy, headers=rss_headers) as resp:
            if resp.status == 200:
...
async def set_feed_config(session: str, url: str, **changes):
    ...
    if changes.get('name'):
        name = changes['name']
    # Pay attention to the special value of key_enable
...
async def fetch_feed_entries(url: str, last_tsp: float):
    latest_tsp = last_tsp
    unsent_entries = []
    ...
    for entry in entries:
        time_parsed = entry['published_parsed'] if not entry.get('updated_parsed') else entry['updated_parsed']
...
async def send(entries: list, session: str, session_name: str,self_id: int):
    self_id = str(self_id)
    bot = nonebot.get_driver().bots[self_id]
...
async def fetch_and_send(session: str, self_id: int):
    ...
    for row in rows:
        ...
        if enable:
            ...
            try:
                unsent_entries, latest_tsp = await fetch_feed_entries(url, tsp)
            ...
            if len(unsent_entries) > 0:
                ...
                await send(unsent_entries, session, name, self_id)
